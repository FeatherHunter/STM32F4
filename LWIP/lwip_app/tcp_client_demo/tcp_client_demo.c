...
void tcp_client_test(u8 * connect_flag, u8 * login, struct tcp_pcb **tcppcb, struct ip_addr *rmtipaddr)
{
	...
	if(*connect_flag == 1)
	{
		if(*login == 0)
		{
			...
			LCD_ShowString(30,100,lcddev.width-30,lcddev.height-190,16,"loging");//提示消息
			while(!(tcp_client_flag&1<<6) && t)t--;
			if(t)
			{
				if(strcmp((char *)tcp_client_recvbuf, "SERVER*toCSlave*LOGIN*OK") == 0)
				{
					LCD_ShowString(30,190,lcddev.width-30,lcddev.height-190,16,"LOGIN");//提示消息
					*login = 1;
				}
		...
		if(*login == 1)
		{
			if(tcp_client_flag&1<<6)//是否收到数据
			{
				...
				tcp_client_flag&=~(1<<6);//标记数据已经被处理了.
				LCD_ShowString(30,250,lcddev.width-30,lcddev.height-190,16,tcp_client_recvbuf);//提示消息
			}
		}
	...
	if(*connect_flag == 0)tcp_client_connection_close(*tcppcb,0);//关闭TCP Client连接
