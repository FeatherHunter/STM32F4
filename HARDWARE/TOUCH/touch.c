 ...
 * @filename touch.c
 * @description 触摸屏驱动（支持ADS7843/7846/UH7843/7846/XPT2046/TSC2046/OTT2001A等） 代码	   
 *								电容触摸屏增加FT5206的支持 
 * @FunctionList
 *    电阻屏/电容屏 共用函数
 *		1.u8 TP_Scan(u8 tp);															//扫描
 *		2.u8 TP_Init(void);																//初始化
 *    电阻屏函数
 *		3.void TP_Write_Byte(u8 num);											//向控制芯片写入一个数据
 *		4.u16 TP_Read_AD(u8 CMD);													//读取AD转换值
 *		5.u16 TP_Read_XOY(u8 xy);													//带滤波的坐标读取(X/Y)
 *		6.u8 TP_Read_XY(u16 *x,u16 *y);										//双方向读取(X+Y)
 *		7.u8 TP_Read_XY2(u16 *x,u16 *y);									//带加强滤波的双方向坐标读取
 *		8.void TP_Drow_Touch_Point(u16 x,u16 y,u16 color);//画一个坐标校准点
 *		9.void TP_Draw_Big_Point(u16 x,u16 y,u16 color);	//画一个大点
 *		10.void TP_Save_Adjdata(void);											//保存校准参数
 *		11.u8 TP_Get_Adjdata(void);												//读取校准参数
 *		12.void TP_Adjust(void);														//触摸屏校准
 *		13.void TP_Adj_Info_Show(u16 x0,u16 y0,u16 x1,u16 y1,u16 x2,u16 y2,u16 x3,u16 y3,u16 fac);//显示校准信息
 */ 
...
/**
 * @Function u8 TP_Init(void)
 * @description 触摸屏初始化  
 *              PB0,PB1,PC13,PF11对应LCD四个端口
 * @Input 	void
 * @Return  u8 0,没有进行校准
 *			       1,进行过校准
 */	
...
/**
 * @Function void TP_Write_Byte(u8 num)  
 * @description SPI写数据
 *							向触摸屏IC写入1byte数据 
 * @Input 	u8 num:要写入的数据;
 * @Return  void
...
/**
 * @Function u16 TP_Read_AD(u8 CMD); 
 * @description SPI读数据 
 *						从触摸屏IC读取adc值
 * @Input 	u8 CMD:指令
 * @Return  u16 读到的数据	;
