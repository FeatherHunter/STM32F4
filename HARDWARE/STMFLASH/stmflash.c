 ...
 * @FunctionList
 *		1.u32 STMFLASH_ReadWord(u32 faddr);		  														//读出字  
 *		2.void STMFLASH_Write(u32 WriteAddr,u32 *pBuffer,u32 NumToWrite);		//从指定地址开始写入指定长度的数据
 *		3.void STMFLASH_Read(u32 ReadAddr,u32 *pBuffer,u32 NumToRead);   		//从指定地址开始读出指定长度的数据   
 *		4.uint16_t STMFLASH_GetFlashSector(u32 addr);                       //获取某个地址所在的flash扇区
 */
