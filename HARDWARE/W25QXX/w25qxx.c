 ...
 *		2.u16  W25QXX_ReadID(void);  	    		//读取FLASH ID
 *		3.u8	 W25QXX_ReadSR(void);        		//读取状态寄存器 
 *		4.void W25QXX_Write_SR(u8 sr);  			//写状态寄存器
 *		5.void W25QXX_Write_Enable(void);  		//写使能 
 *		6.void W25QXX_Write_Disable(void);		//写保护
 *		7.void W25QXX_Write_NoCheck(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite);
 *		8.void W25QXX_Read(u8* pBuffer,u32 ReadAddr,u16 NumByteToRead);   //读取flash
 *		9.void W25QXX_Write(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite);//写入flash
 *		10.void W25QXX_Erase_Chip(void);    	  		//整片擦除
 *		11.void W25QXX_Erase_Sector(u32 Dst_Addr);	//扇区擦除
 *		12.void W25QXX_Wait_Busy(void);           	//等待空闲
 *		13.void W25QXX_PowerDown(void);        		  //进入掉电模式
 *		14.void W25QXX_WAKEUP(void);								//唤醒
 */ 
