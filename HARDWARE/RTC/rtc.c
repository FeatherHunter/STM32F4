 ...
 * @description RTC
 * @FunctionList
 *		1.u8 My_RTC_Init(void);																						//RTC初始化
 *		2.ErrorStatus RTC_Set_Time(u8 hour,u8 min,u8 sec,u8 ampm);				//RTC时间设置
 *		3.ErrorStatus RTC_Set_Date(u8 year,u8 month,u8 date,u8 week); 		//RTC日期设置
 *		4.void RTC_Set_AlarmA(u8 week,u8 hour,u8 min,u8 sec);							//设置闹钟时间(按星期闹铃,24小时制)
 *		5.void RTC_Set_WakeUp(u32 wksel,u16 cnt);													//周期性唤醒定时器设置
 */ 
#include "rtc.h"
#include "led.h"
#include "delay.h"
...
u8 My_RTC_Init(void)
{
  ...
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR, ENABLE);		//使能PWR时钟
    ...
    RTC_Init(&RTC_InitStructure);
		...
		RTC_Set_Time(12, 24, 0, RTC_H12_AM);				//设置时间
 ...
 * @Input    week:星期几(1~7) @ref  RTC_Alarm_Definitions
