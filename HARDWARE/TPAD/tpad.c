 ...
 * @description 电容触摸按键
 * @FunctionList
 *		1.void TPAD_Reset(void);        复位
 *		2.u16  TPAD_Get_Val(void);      得到定时器捕获值
 *		3.u16 TPAD_Get_MaxVal(u8 n);    读取n次,取最大值
 *		4.u8   TPAD_Init(u8 systick);   初始化触摸按键
 *		5.u8   TPAD_Scan(u8 mode);      扫描触摸按键
 *		6.void TIM2_CH1_Cap_Init(u32 arr,u16 psc);   定时器2通道2输入捕获配置	
  */ 
...
/**
 * @Function u8 TPAD_Init(u8 psc);
 * @description 初始化触摸按键,获得空载的时候触摸按键的取值.
 * @Input 	u8 psc:分频系数,越小,灵敏度越高.
...
/**
 * @Function void TPAD_Reset(void)
 * @description 复位一次,释放电容电量，并清除定时器的计数值
 * @Input 	void
 * @Return  void
 */
...
/**
 * @Function u16 TPAD_Get_Val(void)
 * @description 得到定时器捕获值,如果超时,则直接返回定时器的计数值.
 * @Input 	void
 * @Return  捕获值/计数值（超时的情况下返回）
 */
...
/**
 * @Function u16 TPAD_Get_MaxVal(u8 n)
 * @description 读取n次,取最大值
 * @Input 	n：连续获取的次数
 * @Return  n次读数里面读到的最大读数值
 */
...
/**
 * @Function void TIM2_CH1_Cap_Init(u32 arr,u16 psc)
 * @description 定时器2通道2输入捕获配置	
 * @Input 	u32 arr：自动重装值
 *				  u16 psc：时钟预分频数
 * @Return  void
 */
