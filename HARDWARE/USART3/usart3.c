 ...
 * @filename usart3.c
 * @description 串口3可以用于和HC05通信
 * @FunctionList
 *		1.void usart3_init(u32 bound);							//串口3初始化 
 *		2.void TIM7_Int_Init(u16 arr,u16 psc);      //定时器7接受串口数据
 *		3.void u3_printf(char* fmt, ...);           //发送数据
 */
#include "delay.h"
#include "usart3.h"
#include "stdarg.h"	 	 
 ...
 * @description USART3中断处理函数
 *             一次连续的数据:接收连续2个字符时间<=100ms.
 *						 本次数据接收完毕：接收连续2个字符时间>100ms.   
 * 数据状态USART3_RX_STA
 *         [15]:0,没有接收到数据;
 *							1,接收到了一批数据.
 *				 [14:0]:接收到的数据长度 
 ...
 * @description 初始化IO 串口3 
 *              PB10,PB11
 * @Calls
 *    1.TIM7_Int_Init(100-1,8400-1);		//10ms中断一次
 *    2.TIM_Cmd(TIM7, DISABLE); 				//关闭定时器7
 * @Input    u32 bound: 波特率
 * @Return   void
 */
