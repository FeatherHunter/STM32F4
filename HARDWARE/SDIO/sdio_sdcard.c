 ...
 * @Author Feather @version 1.0 @date 2015.12.10
 * @filename sdio_sdcard.c
 * @description SDIO
 * @FunctionList
 *		1.SD_Error SD_Init(void);                      		 //初始化
 *		2.void SDIO_Clock_Set(u8 clkdiv);              		 //SDIO时钟初始化设置
 *		3.SD_Error SD_PowerON(void);                   		 //卡上电
 *		4.SD_Error SD_PowerOFF(void);                  		 //POWER OFF
 *		5.SD_Error SD_InitializeCards(void);           		 //初始化所有卡并进入就绪状态
 *		6.SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo);	 //得到卡信息	  
 *		7.SD_Error SD_EnableWideBusOperation(u32 wmode);   //设置SDIO总线宽度(MMC卡不支持4bit模式)
 *		8.SD_Error SD_SetDeviceMode(u32 mode);             //设置SD卡工作模式
 *		9.SD_Error SD_SelectDeselect(u32 addr);            //选卡
 *		10.SD_Error SD_SendStatus(uint32_t *pcardstatus);   //读取当前卡状态
 *		11.SDCardState SD_GetState(void);                   //返回SD卡的状态
 *		12.SD_Error SD_ReadBlock(u8 *buf,long long addr,u16 blksize);                   //SD卡读取一个块 
 *		13.SD_Error SD_ReadMultiBlocks(u8 *buf,long long  addr,u16 blksize,u32 nblks);  //SD卡读取多个块 
 *		14.SD_Error SD_WriteBlock(u8 *buf,long long addr,  u16 blksize);	               //SD卡写1个块 
 *		15.SD_Error SD_WriteMultiBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks);  //SD卡写多个块 
 *		16.SD_Error SD_ProcessIRQSrc(void);                 	 //SDIO中断处理函数
 *		17.void SD_DMA_Config(u32*mbuf,u32 bufsize,u32 dir);  //配置SDIO DMA  
 *		//void SD_DMA_Config(u32*mbuf,u32 bufsize,u8 dir); 
 *		18.u8 SD_ReadDisk(u8*buf,u32 sector,u8 cnt); 	//读SD卡,fatfs/usb调用
 *		19.u8 SD_WriteDisk(u8*buf,u32 sector,u8 cnt);	//写SD卡,fatfs/usb调用
 */ 
