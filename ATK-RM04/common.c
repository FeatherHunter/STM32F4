 ...
 * @Author Feather @version 1.0 @date 2015.12.12
 * @filename common.c
 * @description ATK-RM04 WIFI模块 公用驱动代码
 * @FunctionList
 *		1.void atk_rm04_init(void);                             //atk_rm04 初始化
 *		2.void atk_rm04_at_response(u8 mode);      							//将收到的AT指令应答数据返回给电脑串口
 *		3.u8* atk_rm04_check_cmd(u8 *str);											//ATK-RM04发送命令后,检测接收到的应答
 *		4.u8 atk_rm04_send_cmd(u8 *cmd,u8 *ack,u16 waittime); 	//向ATK-RM04发送命令
 *		5.u8 atk_rm04_quit_trans(void);													//ATK-RM04退出透传模式
 *		6.u8 atk_rm04_consta_check(void);												//获取ATK-RM04模块的连接状态
 *		7.void atk_rm04_load_keyboard(u16 x,u16 y);							//加载键盘界面（尺寸为240*140）
 *		8.void atk_rm04_key_staset(u16 x,u16 y,u8 keyx,u8 sta); //按键状态设置
 *		9.u8 atk_rm04_get_keynum(u16 x,u16 y);									//得到触摸屏的输入
 *		10.void atk_rm04_get_wanip(u8* ipbuf);										//获取wan ip地址
 *		11.u8 atk_rm04_get_wifista_state(void);									//获取WIFI STA连接状态(检查WIFI STA是否连上路由器)
 *		12.void atk_rm04_msg_show(u16 x,u16 y,u8 wanip);					//ATK-RM04模块信息显示
 *		13.void atk_rm04_wificonf_show(u16 x,u16 y,u8* rmd,u8* ssid,u8* encryption,u8* password);//ATK-RM04模块WIFI配置参数显示(仅WIFI STA/WIFI AP模式测试时使用)
 *		14.u8 atk_rm04_netpro_sel(u16 x,u16 y,u8* name);					//工作模式选择
 *		15.void atk_rm04_mtest_ui(u16 x,u16 y);									//测试界面主UI
 *		16.u8 atk_rm04_ip_set(u8* title,u8* mode,u8* port,u8* ip);//IP设置
 *		17.void atk_rm04_test(void);															 //ATK-RM04模块测试主函数
 *		18.u8 atk_rm04_cometh_test(void);	//串口以太网测试
 *		19.u8 atk_rm04_wifista_test(void);	//WIFI STA测试
 *		20.u8 atk_rm04_wifiap_test(void);	//WIFI AP测试
 */
...
const u8* wifista_ssid="CH";			//路由器SSID号
const u8* wifista_encryption="wpawpa2_aes";	//wpa/wpa2 aes加密方式
const u8* wifista_password="feiniao886"; 	//连接密码
